...
let TIMER_AUTO_TASK_START = "timerAutoTaskStart"
...
function CommonFunctions () {
  ...
  this.cancelAllTimedTasksById = function () {
    let task = storageFactory.getValueByKey(TIMER_AUTO_TASK_START, true).task
    if (task && '{}' != JSON.stringify(task)) {
      _logUtils.debugInfo('撤销自动任务：' + JSON.stringify(task))
      Timers.removeTimedTask(task.taskId);
    }
    // 将task队列置为空
    storageFactory.updateValueByKey(TIMER_AUTO_TASK_START, {})
  }
  /**
   * 设置指定时间后自动启动main脚本
   *
  ...
  this.setUpAutoStart = function (minutes) {
    ...
    this.recordTimedTaskId(task)
  }

  this.recordTimedTaskId = function (task) {
    let taskObj = {
      taskId:task.id,
      scriptPath:task.scriptPath
    }
    storageFactory.updateValueByKey(TIMER_AUTO_TASK_START, {task:taskObj})
  }
  ...
  this.showAllAutoTimedTask = function () {
    let array = storageFactory.getValueByKey(TIMER_AUTO_START, true).array
    _logUtils.debugInfo('所有的任务：' + JSON.stringify(array))
