...
const ProjectCommonFunction = function () {
    ...
    this.setNextClockInTime = function (currentDate,config,isToday) {
        ...
        let clockInTime = ""
        //获取当前时间
        let currentHM = this.getDateForHHMM()
        for (let i = 0; i < 4; i++) {
            ...
            if (startTime < currentHM && currentHM < endTime && isToday) {
                _debugInfo(['开始时间: {}, 结束时间: {} 当前时间: {},此时间范围已经执行打过卡了!!!', startTime, endTime, currentHM])
                index += 2
                continue;
            }
    ...
    this.getDateForHHMM = function () {
        const date = new Date();
        return this.fillZero2Two(date.getHours()) + ":" + this.fillZero2Two(date.getMinutes());
    }
